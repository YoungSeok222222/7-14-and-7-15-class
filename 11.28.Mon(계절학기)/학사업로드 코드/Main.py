...
def Me(opp, turn, opp_prev, opp_last_pattern) :
    # opp(대결할 상대방 플레이어 번호),
    # turn(0이상 9이하, 해당 플레이어와의 현재 진행 턴을 나타냄 총 10회),
    # opp_prev( 이번 게임에서 현재 상대방의 바로 이전 선택을 나타냄 )
    # opp_last_pattern (상대방의 바로 이전 게임 기록)  0 사슴 / 1 토끼 / 2 뱀
    opp_choices = []
    ...
    elif (opp_prev == 0 or opp_prev == 2) and rabbit_count > deer_count and rabbit_count > snake_count:
        my_snake = 0
        return 2

    elif opp_prev == 1 and rabbit_count > deer_count and rabbit_count > snake_count:
        my_snake = 0
        return 1
